...
def add_megagame(request):
    if request.method == 'POST':
        ...
        meg1 = Megagame(uid)
        meg1.create_megagame(type=type,
                             title=title,
                             source=source,
                             editorValue=editorValue,
...
def re_add_megagame_info(request):
    if request.method == "POST":
        ...
        create_time = time.strftime("%Y-%m-%d %H:%M:%S",
                                    time.localtime(time.time()))
        mega1 = Megagame(uid)
        mega1.create_megagame(title=title,
                              source=source,
                              organizers=organizers,
                              type=type,
                              editorValue=editorValue,
...
def re_add_megagame_questions(request):
    if request.method == "POST":
        ...
        return HttpResponseRedirect('/manage/megagame_question_info?uid=' +
                                    uid)
    uid = request.GET.get('uid')
...
def re_megagame_question_info(request):
    if request.method == 'GET':
        ...
        if question_list:
            map(map_tools, question_list)
        return render(
            request, 'admin/megagame_questions_index.html', {
                'question_list': question_list,
                'page': page,
                'megagame_info': temp1,
                'uid': uid,
...
def re_megagame_question_detail(request):
    if request.method == 'GET':
        ...
        return render(request, 'admin/manage_megagame_question_detail.html',
                      {'question': temp})


def re_update_megagame_question(request):
    if request.method == 'GET':
        qid = request.GET.get('qid')
        temp = models.MegagameQuestions.objects.filter(qid=qid).first()
        ...
        qid = request.GET.get('qid')
        temp = models.MegagameInformation.objects.filter(qid=qid).first()
        if temp:
            temp.update()
            dict = {'code': 1, 'msg': 'success'}
