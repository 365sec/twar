...
class UploadConfig(object):
    def __init__(self, PathFormat, UploadFieldName, SizeLimit, AllowExtensions,
                 SavePath, Base64, Base64Filename):
...
def add_watermark(savePath):
    try:
        #判断是否是图片文件
        if not os.path.splitext(savePath)[-1].lower() in [
                '.jpg', '.jpge', '.png', '.bmp'
        ]:
            return
...
def listFileManage(request, imageManagerListPath, imageManagerAllowFiles,
                   listsize):
...
def uploadimageHandler(request):
    ...
    upconfig = UploadConfig(PathFormat, UploadFieldName, SizeLimit,
                            AllowExtensions, SavePath, False, '')
...
def listimageHandler(request):
    ...
    return listFileManage(request, imageManagerListPath,
                          imageManagerAllowFiles, imagelistsize)
...
def ListFileManagerHander(request):
    ...
    return listFileManage(request, fileManagerListPath, fileManagerAllowFiles,
                          filelistsize)
