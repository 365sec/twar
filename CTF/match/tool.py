...
class Megagame():
    def create_megagame(self, type, title, state, source, editorValue,
        ...
        self.file_path = file
        query1 = models.MatchInfo.objects.filter(title=self.title)
        if query1:
            return False
        else:
            temp = models.MatchInfo(
                uid=self.uid,
                type=self.type,
                title=self.title,
                source=self.source,
    ...
    def update_megagame(self, uid, type, title, source, editorValue,
        ...
        query1 = models.MatchInfo.objects.filter(uid=self.uid)
        if query1:
            return False
        else:
            try:
    ...
    def re_megagame_state(self):
        temp = models.MatchInfo.objects.filter(uid=self.uid)
        return temp.state
    ...
    def add_megagame_questions(self, ttype, kind, description, hint, option0,
        ...
        if ttype == 0:  #选择题
            print 0
            add_q = models.Challenge(uid=self.uid,
                                             type=ttype,
                                             qid=qid,
                                             kind=kind,
