...
def re_megagame_detail(request):
    ...
    person1 = Person_Megagame(user_uid)
    state = 1#person1.re_state(megagame_id)
    if state:
        return render(request, 'mega_detail.html',{'state':'已参赛','megagame_detail':temp})
    else:
        return render(request, 'mega_detail.html',{'state':'我要参加','megagame_detail':temp})
...
def re_join_megagame(request):
    if request.method == 'POST':
        ...
        name = temp.get('name')
        person1 = Person_Megagame(user_ID)
...
def re_operation(request):
    if request.method == "GET":
        ...
        print uid
        question_temp = models.MegagameQuestions.objects.filter(uid=uid,type__gt= 1).order_by('id')
        ...
        return render(request, 'megagame_list.html', {'question_list': question_list,'page':page,'num_pages':paginator.num_pages,'uid':uid,'user_ID':user_ID})
...
def re_megagame_choice(request):
    if request.method == 'GET':
        ...
        strquery = json.dumps(list(query.values()),ensure_ascii=False,cls=DateEncoder)
        return render(request, 'megagame_choice.html', {'list': strquery,'question_list':query})
    if request.method == 'POST':
...
def re_megagame_questions_detail(request):
    if request.method == 'GET':
        ...
        if query:
            return render(request, 'megagame_questions_detail.html', {'question': query[0]})
    if request.method == 'POST':
        return  render(request, 'megagame_list.html')
...
def re_megagame_questions_list(request):
    if request.method == 'GET':
        ...
        return render(request, 'megagame_question_list.html', {'questions_list': query})
    if request.method == 'POST':
        return  render(request, 'megagame_list.html')
    ...
    if request.method == 'POST':
        ...
        if answer == query.correction:
            ...
            if  not query1.exists():
                ...
                reply_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
                answer=models.Answer(qid=qid,user_answer=answer,user_ID=user_ID,reply_time=reply_time)
